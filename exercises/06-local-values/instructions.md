...
3. In the file `locals.tf`, define a map named `timeouts` with two key-value pairs. The map defines the key `create` with the value `60m` and the key `delete` with the value `2h`.
